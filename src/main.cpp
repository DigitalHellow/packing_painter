...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (auto& element: elements)
        {
            ...
            glm::vec3 scale = element.r2 ? 
                glm::vec3(2 * element.scale * element.r, 
                    2 * element.scale * element.r2, 0.0) :
                glm::vec3(2 * element.scale * element.r, 
                    2 * element.scale * element.r, 0.0);
            model = glm::scale(model, scale);
            // model = glm::rotate(model, 
            //     (float)(glm::clamp(glfwGetTime() * element.x*element.y, -.5, .5)), 
